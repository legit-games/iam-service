...
const LINK_HISTORY_KEY = ['link-history'];
...
export function useLinkHistory(accountId: string) {
  return useQuery({
    queryKey: [...LINK_HISTORY_KEY, accountId],
    queryFn: () => userApi.getLinkHistory(accountId).then((r) => r.data),
    enabled: !!accountId,
    retry: false,
  });
}
